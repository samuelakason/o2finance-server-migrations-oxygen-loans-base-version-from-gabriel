...
class DataController extends GetxController {
  ...
  late DataVariatiosModel dataVariatiosModel;
  ...
  getProviders() async {
    ...
    update();
    try {
      airtimeProvidersModel = await billRepository.getDataProviders();
  ...
  getDataPlans() async {
    loadingdataPlans = true;
    loadingdataPlansError = false;
    update();
    try {
      dataVariatiosModel =
          await billRepository.getDataVariations(selectedProvider);
      loadingdataPlans = false;
      debugPrint(dataVariatiosModel.message.toString());
      update();
    } catch (e) {
      debugPrint(e.toString());
      loadingdataPlansError = true;
      loadingdataPlans = false;
      update();
      if (e == FetchDataException) {
        AlertMessages().erroMessage("Network Error!",
  ...
  buyData() async {
    ...
    try {
      var response = await billRepository.buyData(
        selectedProvider,
        variationCode,
        phoneCtrl.text.trim(),
      );
